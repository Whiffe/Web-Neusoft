...
$(function() {
	...
	/*打开注册窗口*/
function reg() {
	window.open("register.html", "mywin", "top=150,left=300,width=400,height=300,menubar=no");
}
/*打开登录窗口*/
function login() {
	window.open("login.html", "mywin", "top=150,left=300,width=400,height=300,menubar=no");
}
/*放大镜*/
	...
	$
		.ajax({
			...
			success: function(data) {
				...
				document.getElementById("first_img").src="img/detail/" + data.pic;
				var c = document.getElementById("canvas1");
				var ctx = c.getContext("2d");
				ctx.clearRect(0, 0, c.width, c.height); //擦除画布中原图像
				var img = new Image();
				img.src="img/detail/" + data.pic;
				img.onload=function(){//绘制商品源图
				    ctx.drawImage(first_img, 0, 0, c.width, c.height);
				}
